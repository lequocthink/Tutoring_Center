...
export default function StaffDetailClient({ data, id }: StaffProps) {


    const myId = String(JSON.parse(id).staffId);
    ...
    const onSubmit = (e: FormEvent) => {
        ...
        axios.put(`/api/user/${myId}`, {
            ...
            .then(() => {
                router.refresh()
                router.replace('/admin/staffaccount')
            })
            .catch((err: any) => {
                throw new Error(err)
            })
    ...
    const onDelete = (e: FormEvent) => {
        ...
        const isConfirmed = confirm('Bạn có chắc chắn muốn xóa tài khoản giáo viên ' + state.email + ' ?');
    ...
    return (
        ...
        <div className="">
            <Link href={'/admin/staffaccount/'} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded items-center mt-4 ml-4 absolute">
            ...
            <form className="shadow-[0_3px_10px_rgb(0,0,0,0.2)] p-4" >
                <div className="shadow-[0_3px_10px_rgb(0,0,0,0.2)] mx-auto w-[350px] py-2 mb-[20px]">
                    ...
                    <h1 className="text-center uppercase text-[20px]">Thông Tin Giáo Viên</h1>
                </div>
